...
from core.ssd_fpn_x_creator import create_ssd
...
parser.add_argument('--milestones', default="150", type=str,
...
parser.add_argument('--iou_thresh', default=0.5, type=float,
                    help='iou thresh')
parser.add_argument('--iou_thresh_mid', default=0.35, type=float,
                    help='iou thresh middle')
parser.add_argument('--iou_thresh_low', default=0.1, type=float,
                    help='iou thresh low')
...
parser.add_argument('--model_dir', default='models_fpn_x/',
...
def train(writer, start_step, loader, net, criterion, optimizer, device, debug_steps=100, epoch=-1):
    ...
    for i, data in enumerate(loader):
        images, boxes, objs, objs_mid, objs_low, labels = data
        images = images.to(device)
        boxes = boxes.to(device)
        objs = objs.to(device)
        objs_mid = objs_mid.to(device)
        objs_low = objs_low.to(device)
        labels = labels.to(device)

        optimizer.zero_grad()
        obj_conf, confidence, locations = net(images)
        regression_loss, classification_loss, obj_loss = criterion(obj_conf, confidence, locations, objs, objs_mid, objs_low, labels, boxes)  # TODO CHANGE BOXES
        #classification_loss *= 10.0
        loss = regression_loss + classification_loss + obj_loss
        loss.backward()
        optimizer.step()
        ...
        if i and i % debug_steps == 0:
            ...
            avg_obj_loss = running_obj_loss / debug_steps
            ...
            writer.add_scalar('obj_loss',avg_obj_loss, cur_step)
            logging.info(
...
def test(loader, net, criterion, device):
    ...
    for _, data in enumerate(loader):
        ...
        with torch.no_grad():
            obj_conf, confidence, locations = net(images)
            regression_loss, classification_loss, obj_loss = criterion(obj_conf, confidence, locations, objs, objs_mid, objs_low, labels, boxes)
            #classification_loss *= 10.0
            loss = regression_loss + classification_loss + obj_loss
        ...
        running_classification_loss += classification_loss.item()
        running_obj_loss += obj_loss.item()
    return running_loss / num, running_regression_loss / num, running_classification_loss / num, running_obj_loss / num


if __name__ == '__main__':

    print('train_ssd_fpn_x')
    ...
    target_transform = MatchPrior(priors, center_variance, size_variance, args.iou_thresh, args.iou_thresh_mid, args.iou_thresh_low)
    ...
    training_name = '%s-%dx%d'%(net_type,image_size_x,image_size_y)
    ...
    # create ssd net
    net = create_ssd(net_type, use_gray, num_classes-1, aspect_ratios, priors, center_variance, size_variance, is_test=False, device=DEVICE)
    ...
    if args.freeze_base_net:
        ...
        params = itertools.chain(net.extras.parameters(), net.fpn_convs.parameters(),
                                 net.regression_headers.parameters(), net.classification_headers.parameters(), net.object_headers.parameters())
        params = [
            {'params': itertools.chain(
                net.extras.parameters()
            ), 'lr': extra_layers_lr},
            {'params': itertools.chain(
                net.fpn_convs.parameters(),
                net.regression_headers.parameters(),
                net.classification_headers.parameters()
            )}
        ]
    elif args.freeze_net:
        freeze_net_layers(net.model)
        freeze_net_layers(net.extras)
        params = itertools.chain(net.fpn_convs.parameters(), net.regression_headers.parameters(), net.classification_headers.parameters(), net.object_headers.parameters())
        logging.info("Freeze all the layers except prediction heads.")
    else:
        params = [
            {'params': net.model.parameters(), 'lr': base_net_lr},
            {'params': itertools.chain(
                net.extras.parameters(),
            ), 'lr': extra_layers_lr},
            {'params': itertools.chain(
    ...
    criterion = MultiboxLossX(priors, neg_pos_ratio_mid=3,neg_pos_ratio_low=3,
    ...
    for epoch in range(last_epoch + 1, args.num_epochs):
        ...
        if epoch % args.validation_epochs == 0 or epoch == args.num_epochs - 1:
            val_loss, val_reg_loss, val_cls_loss, val_obj_loss = test(val_loader, net, criterion, DEVICE)
            writer.add_scalar('val_loss', val_loss, epoch)
            writer.add_scalar('val_reg_loss', val_reg_loss, epoch)
            writer.add_scalar('val_cls_loss', val_cls_loss, epoch)
            writer.add_scalar('val_obj_loss', val_obj_loss, epoch)
            logging.info(
                f"Epoch: {epoch}, " +
                f"Valid Loss: {val_loss:.4f}, " +
                f"Valid reg Loss {val_reg_loss:.4f}, " +
                f"Valid cls Loss: {val_cls_loss:.4f}, "
                f"Valid obj Loss: {val_obj_loss:.4f}"
            )
