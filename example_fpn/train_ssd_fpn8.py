...
from core.ssd_fpn.ssd_fpn_base import MatchPrior4
from core.ssd_fpn_creator import create_ssd
from core.datasets.voc_dataset import VOCDataset
from core.datasets.open_images import OpenImagesDataset
from core.multibox_loss import MultiboxLoss7
...
parser.add_argument('--iou_thresh', default="0.5", type=str,
                    help='iou thresh')
parser.add_argument('--iou_thresh_mid', default="0.35", type=str,
...
parser.add_argument('--model_dir', default='models_fpn/',
...
def train(writer, start_step, loader, net, criterion, optimizer, device, debug_steps=100, epoch=-1):
    ...
    for i, data in enumerate(loader):
        images, boxes, labels, labels_mid, labels_low = data
        ...
        regression_loss, classification_loss = criterion(confidence, locations, labels, labels_mid, labels_low, boxes)  # TODO CHANGE BOXES
...
def test(loader, net, criterion, device):
    ...
    for _, data in enumerate(loader):
        ...
        with torch.no_grad():
            confidence, locations = net(images)
            regression_loss, classification_loss = criterion(confidence, locations, labels, labels_mid, labels_low, boxes)
            loss = regression_loss + classification_loss
...
if __name__ == '__main__':
    ...
    target_transform = MatchPrior4(priors, center_variance, size_variance, iou_thresh_list, iou_thresh_mid_list, iou_thresh_low_list)
    ...
    print('iou_thresh_mid_list: ',iou_thresh_mid_list)
    print('iou_thresh_low_list: ',iou_thresh_low_list)
    target_transform.set_iou_threshs(iou_thresh_list, iou_thresh_mid_list, iou_thresh_low_list)
    ...
    # create ssd net
    net = create_ssd(net_type, use_gray, num_classes, aspect_ratios, priors, center_variance, size_variance, is_test=False, device=DEVICE)
    ...
    if args.freeze_base_net:
        ...
        params = itertools.chain(net.fpn_convs.parameters(), net.regression_headers.parameters(), net.classification_headers.parameters())
    ...
    criterion = MultiboxLoss7(priors, neg_pos_ratio_mid=args.neg_pos_ratio_mid, neg_pos_ratio_low=args.neg_pos_ratio_low,
